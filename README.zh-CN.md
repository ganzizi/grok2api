...
> [!NOTE]
> **本 fork（ganzizi/grok2api）已集成出口质量守护。** 默认打开 `qualityGuard` + `requestRetry`（hold 30s、假思考 12h 冷却、空流 15m），`docker compose up -d` 会以 passive 模式启动质量守护 sidecar。默认从本仓库构建镜像，也可通过 `GROK2API_IMAGE` 或 `GROK2API_QG_IMAGE` 指定镜像。
...
- [出口质量守护程序](./tools/egress-quality-guard/README.zh-CN.md)：逐节点模型探测、防误杀隔离和自动恢复；`docker compose up -d` 默认以 passive 模式启动 sidecar
...
`requestRetry` 在网关请求路径上生效，与 sidecar 探测/隔离相互独立。本 fork 默认开启。开启后，可见输出达到 `minOutputTokens` 且全程无流式 reasoning 时**不发给用户**；只有可安全重放的无状态请求才会排除账号重试。TUI 续聊（`previous_response_id`）和 hosted tools 仍会进入 hold 检测，但质量拦截不会把账号绑定状态或有副作用的工具跨账号重放，最终按 `onExhausted` 返回 `503 quality_degraded` 或放出当前响应。上下文压缩、图片、视频和 ForcedEgress 探针不受影响。
