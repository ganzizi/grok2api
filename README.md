...
- [Egress Quality Guard](./tools/egress-quality-guard/README.md) for per-node probes, quarantine, and recovery; it starts with the normal `docker compose up -d` flow
...
`config.example.yaml` includes the qualityGuard settings enabled by default for Build. The main service creates and reuses a non-exportable system probe identity automatically:
